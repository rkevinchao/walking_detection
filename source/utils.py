...
def get_activity_type_for_df(activity):
    # Check activity type
    if activity==1:
        activity_type = "Walking"
    elif activity==2:
        activity_type = "DescStairs"
    elif activity==3:
        activity_type = "AscendStairs"
    elif activity==4:
        activity_type = "Driving"
    elif activity==77:
        activity_type = "Clapping"
    elif activity==99:
        activity_type = "NonStudyAct"
    return activity_type    
...
def get_device_location_for_df(location_id):
    # Check device location
    if location_id=='lw':
        device_location = "WristL"    
    elif location_id=='rw':
        device_location = "WristR" 
    elif location_id=='lh':
        device_location = "HipL"
    elif location_id=='rh':
        device_location = "HipR"        
    elif location_id=='la':
        device_location = "AnkleL"
    elif location_id=='ra':
        device_location = "AnkleR"
    return device_location

def apply_mask(t, data):
    '''
    ...
    mask = t.diff()>threshold             # Find missing periods if the gap is greater than the default sampling rate
    masked_t = ma.masked_where(mask, t)   # Fill gap with '-'
    masked_data = ma.masked_where(mask, data)   # Fill gap with '-'
    return masked_t, masked_data
...
def plot_zoomin(file_id, df, activity, location_id, t0=None, t1=None):
    ...
    x = df2.time_s
    y = df2[loc_x]
    masked_x, masked_y = apply_mask(x, y)
...
def _interpolate(a):
    """
    Interpolote any missing samples
    """
    a = pd.Series(a)
    if pd.isnull(a).any():
        a = a.interpolate(method='linear', limit_direction='both')
    return a.values
